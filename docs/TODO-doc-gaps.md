...
**不要全量通读。** 本文档=「登记表（索引）+ 分节详情」两层：

1. **先看下面的「🗂️ 待办登记表」** —— 一行一个待办，含 **状态 / 优先级 / 分类 / label / 一句话标题 / 是否有截图**。
2. **只想找"还没做的"** → 在登记表里挑 **状态 = 🔴 Open** 的行。
3. **要看某条详情** → 点该行「分类」列跳到对应小节，或直接 **Ctrl-F 它的 `label`**（label 全局唯一）。详情含证据 / 规格 / 修复方向 / 可追溯 AC / 截图。
4. **UI 缺陷有截图** → 见 [§F](#sec-F)，每条 UI bug 配 `docs/reports/uat/screenshots/v45/` 下的截图，文字+图即可看懂。

**状态图例：** 🔴 Open（未做）· 🟡 文档/环境澄清（非阻断）· ✅ Done / Verified（已完成或已验证）
**优先级：** P0 最高 → P2；a11y 用 critical/serious；其余按文中标注。

## 🗂️ 待办登记表（索引）

> 共 25 项。按分类分组；点「分类」跳到对应小节，或 Ctrl-F `label`。

| 状态 | 优先级 | 分类 | label | 一句话标题 | 截图 |
|---|---|---|---|---|---|
| 🔴 | core | [A](#sec-A) | `TODO-online-api-post-optional-sse-runtime-per-step` | 在线 POST 可选 SSE：每步完成后推送 DB 已存 runtime | |
| 🔴 | 依赖↑ | [A](#sec-A) | `TODO-canvas-test-sse-per-step-response-ui-performance` | 画布 Test 用 SSE 逐步展示每步响应 + 大 JSON 性能 | |
| 🔴 | — | [A](#sec-A) | `TODO-deploy-rewrite-online-api-workflow-json` | 重写 Deploy：走 online API + 双块 body + JSON 工作流 | |
| 🔴 | high | [A](#sec-A) | `TODO-e2e-carbon-rewrite-selector-updates` | E2E 选择器更新为 Carbon `.cds--*` | |
| 🟡 | doc | [B](#sec-B) | `TODO-doc-clarify-deploy-step-endpoint-mapping` | Arch 加 Deploy 逻辑三步 ↔ 真实端点对应表 | |
| 🟡 | doc | [B](#sec-B) | `TODO-doc-clarify-history-rollback-client-side` | Arch 注明历史回滚=前端重存语义 | |
| 🟡 | doc | [B'](#sec-Bp) | `TODO-doc-clarify-online-correlation-header-name` | 关联头名统一为 `X-Request-Correlation-Id` | |
| 🟡 | env | [B'](#sec-Bp) | `TODO-tests-online-api-keystore-secret-for-execution` | online-api 完整执行需真实 JKS keystore 密钥 | |
| 🔴 | P0 | [D](#sec-D) | `TODO-online-api-shape-synchronous-response` | 同步响应塑造：调用即拿到结果 | |
| 🔴 | P0 | [D](#sec-D) | `TODO-per-workflow-api-contract-openapi` | 每个 API 的契约 + 自动 OpenAPI | |
| 🔴 | P1 | [D](#sec-D) | `TODO-published-api-auth-ratelimit-cors` | 发布 API 的入站鉴权 / 限流 / CORS | |
| 🔴 | P1 | [D](#sec-D) | `TODO-published-api-custom-routing` | 每个 API 自定义路径 / 方法 | |
| 🔴 | P2 | [D](#sec-D) | `TODO-call-workflow-node-and-catalog` | "调用另一个工作流"节点 + API 目录 | |
| 🔴 | P0 | [E](#sec-E) | `TODO-ui-surface-docs-whatsnew-help` | UI What's New / 更新提示（CHANGELOG 驱动） | |
| 🔴 | P1 | [E](#sec-E) | `TODO-ui-docs-help-page` | UI Docs/Help 页（升级 about） | |
| 🔴 | P1 | [E](#sec-E) | `TODO-ui-contextual-help-links` | UI 上下文帮助（节点/规则/空状态） | |
| 🔴 | P1 | [E](#sec-E) | `TODO-ui-call-this-api-panel` | UI 每个 API 的"如何调用"面板 | |
| 🔴 | critical | [F](#sec-F) | `TODO-ui-a11y-records-form-labels` | /records 表单控件缺标签 | 📷 |
| 🔴 | serious | [F](#sec-F) | `TODO-ui-a11y-canvas-color-contrast` | 画布文字对比度不足 | 📷 |
| 🔴 | serious | [F](#sec-F) | `TODO-ui-a11y-mobile-nav-contrast` | 移动端导航激活态对比度不足 | 📷 |
| 🔴 | usability | [F](#sec-F) | `TODO-ui-modal-esc-close-wai-aria` | 模态支持 Esc 关闭（WAI-ARIA） | 📷 |
| 🔴 | minor | [F](#sec-F) | `TODO-ui-drawer-done-disable-on-rule-error` | 抽屉 Done 未随规则错误禁用（CV-AC-50-4） | 📷 |
| 🔴 | — | [F](#sec-F) | `TODO-tests-canvas-mobile-and-uat-flows` | 画布移动端覆盖 + `@uat` 真后端流程（断言空壳） | 📷 |
| 🔴 | — | [F](#sec-F) | `TODO-tests-ci-wiring-gate` | CI 接线（`--grep @gate` 合并门禁） | |
| ✅ | env | [C](#sec-C) | `TODO-tests-egress-allowlist-uat-hosts` | 放行 UAT egress 以实跑 `tests/`（已验证 2026-06-21） | |

> 截图统一在 `docs/reports/uat/screenshots/v45/`，并随对应 §F 条目内联展示；UAT 实跑结论见 `docs/reports/uat/uat-report-v45.0.md`。

## <a id="sec-A"></a>A. 实现缺口（审计确认未实现/未完成）
...
## <a id="sec-B"></a>B. 文档↔实现偏差（建议文档澄清，非阻断）
...
## <a id="sec-Bp"></a>B'. 本地实跑新增（2026-06-21）
...
## <a id="sec-D"></a>D. 产品功能差距 —— "Serverless Easy API Maker" 定位（roadmap，2026-06-21）
...
## <a id="sec-E"></a>E. UI 体现文档更新（用户文档 → 产品内可见，2026-06-21）
...
## <a id="sec-F"></a>F. 测试体系发现的真实缺陷（2026-06-21，黑盒套件 `tests/` 实跑抓到）

> 由新建的可用性/无障碍套件实跑发现，均为 workflow-ui 缺陷（走完整流程修复）。已在 `tests/` 用基线豁免/`@advisory` fixme 标记，修复后移除标记即转回归。
>
> **截图证据：** 下列每条 UI bug 配截图于 `docs/reports/uat/screenshots/v45/`（对真实 UAT `workflow-ui-gamma.vercel.app` 实拍）。复现脚本：`tests/scripts/capture-bug-screenshots.mjs`。
...
- [ ] **UI 无障碍 — /records 表单控件缺标签** *(label: `TODO-ui-a11y-records-form-labels`)* — **Status:** Open（critical）。**证据:** axe WCAG 2.2 AA 扫 `/records`：`label` ×2、`select-name` ×1（表单输入/下拉无可访问名）。**修复:** 给筛选输入/下拉补 `<label>`/`aria-label`。**追溯:** `tests/a11y/records.a11y.spec.ts` 基线豁免。
  ...
  ![/records 筛选控件无标签](reports/uat/screenshots/v45/records-a11y-form-labels.png)
  *图：`/records` 顶部筛选输入/下拉无 `<label>`/`aria-label`，屏幕阅读器读不出名字（axe: label×2 + select-name×1）。*
...
- [ ] **UI 无障碍 — 画布文字对比度不足** *(label: `TODO-ui-a11y-canvas-color-contrast`)* — **Status:** Open（serious）。**证据:** axe 扫画布：`color-contrast` ×6，文字/背景对比度 < 4.5:1。**修复:** 调整相关文字颜色达 WCAG AA。**追溯:** `tests/a11y/canvas.a11y.spec.ts` 基线豁免。
  ...
  ![画布文字对比度不足](reports/uat/screenshots/v45/canvas-color-contrast.png)
  *图：画布节点调色板/节点说明等浅灰文字与白底对比度 < 4.5:1（axe color-contrast×6）。*
...
- [ ] **UI 无障碍 — 移动端导航激活态文字对比度不足** *(label: `TODO-ui-a11y-mobile-nav-contrast`)* — **Status:** Open（serious）。**证据:** axe 扫移动视口（390px）多页：移动 tab bar 激活态链接文字 `color-contrast` < 4.5:1（共享 nav，跨 /workflows、/、/records）。**修复:** 调整移动激活态文字/背景色达 WCAG AA。**追溯:** `tests/a11y/*` 移动端基线豁免。
  ...
  ![移动端导航激活态对比度不足](reports/uat/screenshots/v45/mobile-nav-contrast.png)
  *图：390px 移动端底部 tab bar，激活态（Applications，蓝色）文字与深色底对比度 < 4.5:1；该 nav 跨 /workflows、/、/records 共享。*
...
- [ ] **UI 可用性 — 模态支持 Esc 关闭（WAI-ARIA 对话框模式）** *(label: `TODO-ui-modal-esc-close-wai-aria`)* — **Status:** Open（usability）。**证据:** 新建/导入等模态有 ×/Cancel 但**无 Esc 关闭**（违反 WAI-ARIA Authoring Practices 对话框模式）。**修复:** 模态统一加 Esc 关闭处理。**追溯:** `tests/ux/*` `@advisory` fixme。
  ...
  ![模态无 Esc 关闭](reports/uat/screenshots/v45/modal-no-esc-close.png)
  *图：Import 模态打开态，有 ×/Cancel 但按 Esc 不关闭（违反 WAI-ARIA 对话框模式）。*
...
- [ ] **UI 画布 — 节点抽屉 Done 未随规则错误禁用（CV-AC-50-4）** *(label: `TODO-ui-drawer-done-disable-on-rule-error`)* — **Status:** Open（轻微）。**证据:** 画布全功能 E2E：节点抽屉编辑态规则键填非法 JSONPath（如 `a, b`）会显示内联红错（"single JSONPath expression"），但提交按钮 **Done 未被禁用**；CV-AC-50-4 要求"验证错误时禁用保存按钮"。**修复:** 任一规则键有错时禁用 Done。**追溯:** `tests/e2e/canvas-full.spec.ts` E2（@advisory fixme）。
  ...
  ![抽屉 Done 未随规则错误禁用](reports/uat/screenshots/v45/drawer-done-not-disabled-on-rule-error.png)
  *图：规则键填非法 JSONPath「a, b」→ 字段下出现红错「Rule key must be a single JSONPath expression」，但右上 **Done** 仍是可点的蓝色按钮（应被禁用）。*
...
- [ ] **测试 — 画布富交互的移动端覆盖 + UAT 真后端流程** *(label: `TODO-tests-canvas-mobile-and-uat-flows`)* — **Status:** Open（2026-06-21 实跑确认，证据更具体）。**目标:** (1) 移动端画布工具栏在溢出菜单内，需补"打开溢出菜单"以解锁移动端的 import/配置/删除等富交互 E2E；(2) `@uat` 标记的真后端流程（Run 真执行返回摘要、Save 持久化重载、AI Explain/Generate 真返回）需 UAT egress 放行后用 `RUN_UAT=1` 实跑。**实跑发现（egress 已放行后）:** `@uat G2`（Run 真执行）与 `@uat I2`（Save 持久化）当前是**空壳/无断言**——`I2` body 为空（仅 `test.skip`），`G2` 点 Run 后无任何断言（验证仅为注释）。因此桌面端 `RUN_UAT=1` 下二者**平凡通过**（不真正验证后端）；移动端 `@uat G2` 反而**超时失败**，因为 Run 按钮在移动溢出菜单内、用例未先打开溢出菜单。**修复:** (a) 给 `@uat` 用例补真断言（Run 后断响应摘要可见/含状态；Save 后重载断节点持久化）；(b) `@uat` 富交互用例加视口守卫或先开溢出菜单，避免移动端误失败。**追溯:** `tests/e2e/canvas-full.spec.ts`；详见 `docs/reports/uat/uat-report-v45.0.md`。
  ...
  ![移动端 Run 在溢出菜单内](reports/uat/screenshots/v45/mobile-run-in-overflow.png)
  *图：390px 移动端点 `⋯` 溢出菜单后，Straighten/Explain/Generate/Import/JsonPath/**Run** 全在菜单内；`@uat G2` 直接找 Run 未先开菜单 → 30s 超时（测试缺陷，非产品缺陷）。*
