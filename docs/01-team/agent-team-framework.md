...
> 换项目时**只改 [§9 项目绑定表](#binding)**，其余章节原样复用。
>
> **快速导航：** [摘要](#abstract) · [原则](#principles) · [编排与数据流](#orchestration) · [质量门](#gates) · [成熟度](#maturity) · [角色定义卡](#roles) · [文档生命周期](#lifecycle) · [落地建议](#adoption) · [项目绑定表](#binding)
...
框架本身**不绑定任何技术栈、仓库或领域**。所有项目特有信息（服务/仓库矩阵、技术栈、文档根路径、领域代码、环境地址）都以占位符 `{{…}}` 出现，集中在 **[§9 项目绑定表](#binding)**。实例化一个新项目 = 填写绑定表 + 复制角色定义卡。
...
> 卡内**不含**项目特有的仓库名、技术栈、领域码——这些以占位符引用 [§9 项目绑定表](#binding)。
...
| **典型输出** | 版本化产品文档切片 `{{PM_SLICE_PATTERN}}`（用户故事、验收标准、变更摘要）；产品主基线 `{{PM_MASTER}}`（按 `{{DOMAIN_CODES}}` 组织，**不写**接口/错误码——那归架构文档） |
...
| **工具** | 界面：手工探索 + 可选 `{{E2E_TOOL}}`；接口：HTTP 客户端、契约/集合测试工具、CI 报告附件 |
...
| 验收环境 | 前端 https://workflow-ui-gamma.vercel.app；operation/online 各自 base URL |
...
| `{{DOC_ROOT}}` | `workflow-agent-teams/docs/` |

---

> **维护说明：** 本文件是**通用框架**；项目特有内容只应出现在 §9。若发现正文出现了具体仓库名/技术栈/路径而非占位符，应回填到 §9 并在正文改回占位符，以保持可复用性。
