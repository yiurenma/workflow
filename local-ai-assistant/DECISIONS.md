...
- **内存单条 16GB（单通道）。** 原建议补成 32G 双通道；但 2026 年 DDR5 涨价（单条 ~¥1200），**暂缓**：
  模型在显存里、网页用 pip 免 Docker，16G 单条够起步；价格回落或真遇内存吃紧再补。
...
### 为什么是 Qwen，而不是 GPT-4/Claude 等国外顶级模型
约束是「**能在 16G 显卡上本地跑**」，不是「全世界最强」。这个前提决定了选择：
- **国外顶级模型（GPT-4o / Claude / Gemini）是闭源、只能 API 调用**，跑在别人数据中心，**下载不到、装不进你的显卡**。用它们=联网 + 按 token 付费 + 数据出境，与本项目「本地/私密/免费/可离线/给用户随便用不计费」的初衷相反。
- **在"能本地跑"的范围里（开源、≤14B 塞进 16G）**：同尺寸国外开源模型 Llama 3.1 8B / Gemma 2 9B / Mistral Nemo 12B 等中文偏弱、代码也不占优；**Qwen2.5-14B 在"中文 + 代码"这一档是最强之一**（DeepSeek 同为中国模型、亦强）。即"本地小模型 + 中文 + 代码"最优解恰是中国模型。
- **没被锁死**：网关支持「本地 Qwen 打底 + 硬核任务经 VPN 调云端 Claude/DeepSeek 兜底」的混合用法。
- **想要更强的本地**：需加显存（第二张卡/更大卡）才能上 32B/70B —— Z790 + 1200W 就是给这条路预留的。
- **一句话**：选 Qwen 不是"它最强"，而是"在『本地 16G + 中文 + 代码 + 免费私密』这套约束下它最优"。
