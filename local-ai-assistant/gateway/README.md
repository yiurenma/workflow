...
> ⚠️ **常见坑：换 nameserver ≠ 改 DNS 记录。** 它在域名的**独立「Nameservers」设置**里，**不在** DNS 记录表里。
> - **GoDaddy**：域名 → **Nameservers → Change → "I'll use my own nameservers"** →
>   把 `ns49/ns50.domaincontrol.com` **整体替换**成 Cloudflare 分配的两个（如 `finley.ns.cloudflare.com` / `galilea.ns.cloudflare.com`）→ Save。
> - 记录表里的 `ns*.domaincontrol.com` 显示「不能删」是**正常**的，**不用删**；在 DNS 记录表里加 Cloudflare 的 NS 记录**无效**。
> - 若该域名有在用网站（@ 的 A 记录、www 的 CNAME），换 NS 前确认 Cloudflare 加站点时**已扫描导入**这些记录，别漏。
